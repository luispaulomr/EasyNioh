...
void CHandleProcess::WriteMemory(uintptr_t addr, const std::vector<uint8_t>& to_write)
{
        ...
        // modify page permissions! VirtualQueryEx and VirtualProtectEx
        DWORD oldprotect;
        ...
        if (!VirtualQueryEx(m_hProcess, (LPVOID) addr, &mbi, sizeof(mbi))) {
                std::cout << "[CHandleProcess::WriteMemory] ERROR in VirtualQueryEx" << "\n";
                return;
        }

        if (mbi.Protect != PAGE_EXECUTE_READWRITE && mbi.State == MEM_COMMIT) {

                if (!VirtualProtectEx(m_hProcess, mbi.BaseAddress, mbi.RegionSize, PAGE_EXECUTE_READWRITE, &oldprotect)) {
                        std::cout << "[CHandleProcess::WriteMemory] ERROR in VirtualProtectEx: " << GetLastErrorAsString() << "\n";
                        return;
                }

        } else if (mbi.State == MEM_FREE) {

                if (!VirtualAllocEx(m_hProcess, mbi.BaseAddress, mbi.RegionSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)) {
                        std::cout << "[CHandleProcess::WriteMemory] ERROR in VirtualAllocEx: " << GetLastErrorAsString() << "\n";
                        return;
                }
        }
        ...
        if (bytes_written != to_write.size()) {
                std::cout << "[CHandleProcess::WriteMemory] ERROR: Could not write to the requested memory" << "\n";
                return;
        }
}
...
uintptr_t CHandleProcess::_FindExternalPattern(HANDLE h_proc, const char* pattern, uintptr_t begin, intptr_t size)
{
        ...
        for (uintptr_t curr = begin; curr < begin + size; curr += mbi.RegionSize) {
                ...
                if (mbi.State != MEM_COMMIT || mbi.Protect == PAGE_NOACCESS || mbi.Protect != PAGE_EXECUTE_READ) {
                        continue;
                }
